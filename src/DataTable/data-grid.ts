...
export class dataGrid {
    static setSortIcon(component: DataTable, index) {
    ...
    static setPerPageOptions(component: DataTable) {
        let { currentPerPage, defaultPerPage, perPage } = component.get();
        let options = perPage;
    
        // Force numbers
        options = options.map(v => typeof v === 'string' ? parseInt(v, 10) : v);
    ...
    static exportExcel(component: DataTable) {
    ...
    static print(component: DataTable) {
        let win = window.open("");
    ...
    static renderTable(component: DataTable) {
    ...
    static click(component: DataTable, row) {
